	...
	Если СтрокаТ <> Неопределено Тогда
		...
		ИначеЕсли Элемент.ТекущийЭлемент.Имя = "СоставСоглашение" Тогда
			Если ЗначениеЗаполнено(СтрокаТ.Соглашение) Тогда
				...
				СтррЗначения = ЗначенияРеквизитовОбъекта(СтрокаТ.Соглашение, "Организация, Владелец");
				СтрокаТ.Организация = СтррЗначения.Организация;
				
				Если ЗначениеЗаполнено(СтррЗначения.Владелец) Тогда
					СтрокаТ.Партнер 	= СтррЗначения.Владелец;				
				КонецЕсли; 
				
				//Замена
				//СтррЗначения = ЗначенияРеквизитовОбъекта(СтрокаТ.Соглашение, "Партнер,Организация,Контрагент"); //возможно я изменил уже
				//СтрокаТ.Организация = СтррЗначения.Организация;
				//
