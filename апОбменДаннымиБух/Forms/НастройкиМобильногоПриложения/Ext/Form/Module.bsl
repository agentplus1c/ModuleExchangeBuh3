...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Не Параметры.Свойство("ТорговыйПредставитель") И Не Параметры.Свойство("НастройкиМобильногоПриложения") Тогда
		ВызватьИсключение("Не заданы свойства Параметры.ТорговыйПредставитель и Параметры.НастройкиМобильногоПриложения.");
	КонецЕсли;
...
Процедура ДобавитьНастройкуВТаблицу(Ветка, СтррНастройка, ЭтоГруппа, ТолькоПросмотр, ЗаполнитьПоУмолчанию)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Структура") Тогда // Для значений типа "Структура" должно быть свое представление
		СтрокаД.ВнутреннееЗначение = Значение;
		СтрокаД.Значение = ПолучитьПредставлениеСтруктурыНастройки(СтрокаД.Ключ, СтрокаД.ВнутреннееЗначение);
	Иначе
...
Функция ДеревоНастроекЗаполнить(ЗаполнитьПоУмолчанию)
	...
	//(( sk_190724 Удален неиспользуемый макет настроек мобильного приложения и переименован текущий
	//МакетНастроек = РеквизитФормыВЗначение("Объект").ПолучитьМакетОбработки("НастройкиМобильногоПриложения1");
	МакетНастроек = РеквизитФормыВЗначение("Объект").ПолучитьМакетОбработки("НастройкиМобильногоПриложения");
	//)) sk_190724
	//}}dm_180523
	НомерСтроки = 2;
...
&НаСервере 
Функция ПолучитьПредставлениеСтруктурыНастройки(Ключ, СтррЗначение)
	...
	Иначе
		Результат = ТекОбъект.ПолучитьПредставлениеСтруктуры(СтррЗначение);
	КонецЕсли;
...
Процедура ОбновитьПредупреждениеОНастройкахОбмена()
	...
	Если СтрокаД.Значение = Истина Тогда
		ИспользоватьСкладыВТЧДокументовПродажи = Ложь; //КонстантыПолучитьЗначение("ИспользоватьСкладыВТабличнойЧастиДокументовПродажи");
