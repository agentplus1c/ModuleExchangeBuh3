...
Процедура ПолеБраузераПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	//(( sk_190724 Получаем координаты в рамках (MOD-681)
...
Процедура СохранитьКоординаты()
	...
	Для каждого СтрокаТ Из Партнеры Цикл
		Если СтрокаТ.КоординатыИзменены Тогда
			...
			//{{dm_180510:dm_180523
			Если гКонфигурация() = "Бухгалтерия_KZ" И ПоколениеКонфигурации("<=Бухгалтерия_3.0.5.11") Тогда
				ТекОбъект.ЗаписатьСвойстваУОбъекта(СтрокаТ.Партнер, тзРеквизиты);
			Иначе
				//УправлениеСвойствами.ЗаписатьСвойстваУОбъекта(СтрокаТ.Партнер, тзРеквизиты);
				ОбщийМодуль.ЗаписатьСвойстваУОбъекта(СтрокаТ.Партнер, тзРеквизиты);
			КонецЕсли;
			//}}dm_180510
			СтрокаТ.КоординатыИзменены = Ложь;
		КонецЕсли;
...
&НаСервере
Функция КонтактнаяИнформацияОбъектов(СсылкиИлиОбъекты, Знач ТипыКИ = Неопределено, Знач ВидыКИ = Неопределено, Дата = Неопределено) Экспорт

	стррЛитералы = Новый Структура("УсловиеТип, УсловиеВид");
	стррЛитералы.УсловиеТип = ?(НЕ ТипыКИ = Неопределено, "И КонтактнаяИнформация.Тип В (&Тип)", "");
	стррЛитералы.УсловиеВид = ?(НЕ ВидыКИ = Неопределено, "И КонтактнаяИнформация.Вид В (&Вид)", "");
	...
	ТекстЗапроса = 
		...
		|ГДЕ
		|	КонтактнаяИнформация.Объект В (&ОбъектЗначений)
		|	[УсловиеТип]
		|	[УсловиеВид]";
	
	ТекОбъект = РеквизитФормыВЗначение("Объект");
	ТекОбъект.ЗаменитьЛитералы(ТекстЗапроса, стррЛитералы);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("ОбъектЗначений", СсылкиИлиОбъекты);
	Запрос.УстановитьПараметр("Тип", ТипыКИ);
...
Функция ПоколениеКонфигурации(СтрКонфигурация)
	ТекОбъект = РеквизитФормыВЗначение("Объект");
	Возврат ТекОбъект.ПоколениеКонфигурации(СтрКонфигурация);
КонецФункции;
