...
Функция ЭтоПравильныйИдентификатор(GUID, ОписаниеОшибки)
	...
	Для Поз = 1 По 7 Цикл
		GUID = СтрЗаменить(GUID, Сред("АБВСДЕФ", Поз, 1), Сред("ABBCDEF", Поз, 1));
	КонецЦикла;
	...
	Если GUID0 <> "00000000-0000-0000-0000-000000000000" Тогда
		ОписаниеОшибки = НСтр("ru = 'Введите идентификатор в формате XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX, где вместо ""X"" указываются цифры 0-9 и символы A-F (или a-f).'");
		Возврат Ложь;
