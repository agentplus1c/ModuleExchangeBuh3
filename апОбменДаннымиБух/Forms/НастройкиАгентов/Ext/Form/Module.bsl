...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтррКонтекст = Новый Структура("ПрежниеЗначенияСтроки,АвтоКаталог"); // общие значения модуля формы
...
Процедура ПриОткрытии(Отказ)
	...
	//dm
	ЗаполнитьСписокВыбораПодразделения(Элементы.НастройкиАгентовОсновнаяОрганизация.ТекстРедактирования);
	//dm
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "АПНастройкиПараметровОбменаИзмененияЗаписаны" Тогда // Gri++ 18.07.17 записаны настройки модуля, могла быть изменена настройка ТорговляСКолес
		...
		ВосстановитьНастройкиСервер("ТорговляСКолес");
		ОбновитьСтатусТорговлиСКолес();
...
Процедура КомандаНастроитьМобильноеПриложение(Команда)
	...
	Иначе
		...
		ОткрытьФорму(СтррКонтекст.ПутьКФорме + "НастройкиМобильногоПриложения", ПараметрыФормы, ЭтаФорма);
	КонецЕсли;
	...
	Для Каждого ЭлементСписка Из КопируемыеНастройкиДляВыбора Цикл
		Если ЭлементСписка.Пометка Тогда
			Если ЭлементСписка.Значение = "ОсновныеЗначения" Тогда
				Значение = Новый Структура("СсылкаСписокТТ,ОсновнаяОрганизация,ОсновнойСклад,ОсновнойТипЦены");
...
Процедура НастройкиАгентовОсновноеЗначениеПриИзменении(Элемент)
	УстановитьМодифицированостьФормы(Истина);
	//{{dm_180428
	ЗаполнитьСписокВыбораПодразделения(Элемент.ВыделенныйТекст);
КонецПроцедуры
...
&НаКлиенте
Процедура СкладыПриИзменении(Элемент) Экспорт
...
Процедура КомпоновщикНастроекПользовательскиеНастройкиПриИзменении(Элемент)
	...
	//Если Элемент.ТекущиеДанные.Настройка = "Партнеры" Тогда
	//	стррЭлемент = Новый Структура("ВидСравнения, Значение");
	//	стррЭлемент.Вставить("ВидСравнения", Строка(Элемент.ТекущиеДанные.ВидСравнения));
	//	стррЭлемент.Вставить("Значение", Элемент.ТекущиеДанные.Значение);
	//	стрЗначения = ПартнерыВзаимоисключение(стррЭлемент);
	//КонецЕсли;
...
Процедура ЗагрузитьНастройкиВыгружаемыхДанных()
	...
	ТекОбъект = РеквизитФормыВЗначение("Объект");
	//++Дейнеко ВГ 2017-09-11 Бухгалтерия
	СхемаКомпоновки = ТекОбъект.ПолучитьМакет("СКДВыгружаемыеДанныеДЛЯБух31"); //dm:Замена ("СКДВыгружаемыеДанныеДЛЯБух30")
	//Замена
	//СхемаКомпоновки = ТекОбъект.ПолучитьМакет("СКДВыгружаемыеДанные");
	//--Дейнеко ВГ 2017-09-11 Бухгалтерия
...
//{{dm_180428
&НаСервере
Процедура ЗаполнитьСписокВыбораПодразделения(НаменованиеОрганизации)
	
	ТекОбъект = РеквизитФормыВЗначение("Объект");
	
	Если ЗначениеЗаполнено(НаменованиеОрганизации) Тогда
		Организация = Справочники.Организации.НайтиПоНаименованию(НаменованиеОрганизации);
	Иначе
		Организация = Неопределено;
	КонецЕсли;
	
	Список = Элементы.НастройкиАгентовПодразделениеОрганизации.СписокВыбора;
	...
	Пока Выборка.Следующий() Цикл
		Список.Добавить(Выборка.Ссылка, Выборка.Ссылка.Наименование);
	КонецЦикла;
...
//&НаСервере
//Функция ПартнерыВзаимоисключение(Элемент)
//	
//	А = Элемент;
//	//ИД = ЭтаФорма.КомпоновщикНастроек.ПользовательскиеНастройки.ПолучитьИдентификаторПоОбъекту(Справочники.Контрагенты);
//	ЭлементыН = ЭтаФорма.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы;
//	Для каждого Строка Из ЭлементыН Цикл
//		Если Строка(Строка.ЛевоеЗначение) = "Партнеры" Тогда
//			Поиск = Строка;
//			Прервать;
//		КонецЕсли;
//	КонецЦикла;
//	//Поиск = Элементы.Найти(ИД);
//	Возврат "НекийПартнер"
//	
//КонецФункции
