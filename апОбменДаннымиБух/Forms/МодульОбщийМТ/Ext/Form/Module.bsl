...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//{{dm_180618
	СтррКонтекст.Вставить("ИспользоватьДоговорыСКлиентами", Истина);
...
Процедура ФормаОбновитьАдресПартнера(Модуль) Экспорт
	...
	ОбновитьАдресПартнераСервер(Модуль.Контрагент, стррАдрес);
...
Процедура ОбновитьАдресПартнераСервер(Партнер, стррАдрес)
	...
	Если Конфиг.Конфигурация = "Бухгалтерия_KZ" Тогда
		мОбъект = Новый Массив;
		мВидыКИ = Новый Массив;
		мОбъект.Добавить(Партнер);
		мВидыКИ.Добавить(Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента);
		тАдрес = ТекОбъект.КонтактнаяИнформацияОбъектов(мОбъект,, мВидыКИ);
		АдресПартнера = ?(тАдрес.Количество() > 0, тАдрес[0].Представление, "");
	Иначе
		УправлениеКИ = Вычислить("УправлениеКонтактнойИнформацией");
		АдресПартнера = УправлениеКИ.КонтактнаяИнформацияОбъекта(Партнер, Справочники.ВидыКонтактнойИнформации.ФактАдресКонтрагента);//.АдресПартнера);
	КонецЕсли;
	//}}dm_180620
...
Процедура КонтрагентПриИзменении(Модуль, Элемент) Экспорт
	...
	ПроверитьИОчиститьРеквизиты(Модуль, стрРеквизиты, "Ссылка", Модуль.Контрагент);
...
Функция ПолучитьЗначенияПоУмолчанию() 
	...
	ТекОбъект = РеквизитФормыВЗначение("Объект");
	Конфиг = ТекОбъект.ВерсияКонфигурации();
	ОбщийМодуль = ?(Конфиг.Конфигурация = "Бухгалтерия_KZ", Вычислить("РегламентированнаяОтчетностьПереопределяемый"), Вычислить("РегламентированнаяОтчетность"));
	//}}dm_180620
	...
	//стррРезультат.Организация = РегламентированнаяОтчетность.ПолучитьОрганизациюПоУмолчанию();//  ЗначениеНастроекПовтИсп.ПолучитьОрганизациюПоУмолчанию();
	стррРезультат.Организация = ОбщийМодуль.ПолучитьОрганизациюПоУмолчанию();//dm_180620
...
Процедура ВОПриОткрытии(Модуль) Экспорт
	...
	Если стррМКонтекст.ЭтоФормаСписка И стррМКонтекст.ДляВыбора = Истина Тогда
		...
		Если мСтроки.Количество() <> 0 Тогда
			Модуль.Элементы.Документы.ТекущаяСтрока = мСтроки[0].ПолучитьИдентификатор();
		КонецЕсли; 
...
Процедура ВОЭлементыПередПометкойУдаленияЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Массив, Модуль.Элементы.Документы.ВыделенныеСтроки);
...
Функция ВОЭлементыСвойстваТекущего(Модуль) Экспорт
	...
	СтрокаТ = Модуль.Элементы.Документы.ТекущиеДанные;	
...
Процедура ВОЭлементыВыбор(Модуль) Экспорт
	...
	Если Модуль.СтррКонтекст.ДляВыбора = Истина Тогда
		ВОВыбратьИЗакрыть(Модуль, Модуль.Элементы.Документы.ВыделенныеСтроки);
	Иначе
...
Функция ВОПолучитьВыбранные(Модуль, мСтроки) Экспорт
	...
	Элемент = Модуль.Элементы.Документы;
...
Процедура КомандаВыполнить(Команда, Модуль) Экспорт
	...
	Если КомандаИмя = "УдалитьОкончательно" Тогда
		ВОЭлементыПередУдалением(Модуль, Модуль.Элементы.Документы);
